...
label <- read.delim('../data/monocle_label_1.txt', sep = '\t', header = TRUE, row.names = 1, check.names = FALSE)
head(label)
...
label <- label[colnames(expr), ]
label <- label[, c(1:23, ncol(label))]
head(label)


fd <- data.frame(row.names=rownames(expr), gene_short_name=rownames(expr)); head(fd)

opmethod <- read.delim('../data/monocle_clinical.txt', sep = '\t', header = TRUE, row.names = 1, check.names = FALSE)
rownames(opmethod) <- unlist(lapply(rownames(opmethod), function(x) paste0(c('Cat_',x), collapse = '')))
rownames(opmethod)[10] <- 'Cat_TG1-017r'
rownames(opmethod)[34] <- 'Cat_TG2-035r'
nrow(opmethod); length(intersect(rownames(label), rownames(opmethod)))
opmethod <- opmethod[intersect(rownames(label), rownames(opmethod)), c("Operational method", "Pretreatment")]
head(opmethod)
label$op <- 'Not applicable'
label[rownames(opmethod), 'op'] <- as.character(opmethod$`Operational method`)
label$pretx <- 'Not applicable'
label[rownames(opmethod), 'pretx'] <- as.character(opmethod$Pretreatment)
head(label)
...
pData(monocleObj)$site <- mapvalues(pData(monocleObj)$Grade, 
                                    from = levels(pData(monocleObj)$Grade),
                                    to = c('Normal', 'FibCS', 'DN', 'Tumor', 'Tumor', 'Tumor', 'Tumor'))
...
immcluster <- read.delim('../data/monocle_label_2.txt',row.names = 1)
head(immcluster)
identical(sort(rownames(immcluster)), sort(rownames(pData(monocleObj))))
immcluster <- immcluster[rownames(pData(monocleObj)),]

pData(monocleObj)$ImmuneCluster <- immcluster$ImmuneCluster
head(pData(monocleObj))
